...
def main():
    ...
    try:
        importtojson("https://docs.google.com/spreadsheets/d/16X5QlpYoW5aToM6LoJlEZ8K0XvmlasJArD0vYzFOZ3Y/export?format=csv&gid=457897588", target)
...
def startWebview(hotkey):
    class API:
        def close(self):
            webview.windows[0].destroy()

    window = webview.create_window("Izaki Keyboard", "index.html", width=630, height=310, frameless=True, on_top=True, js_api=API())
    threading.Thread(target = keyboardListen, args = (window, hotkey), daemon = True).start()
    webview.start()

def keyboardListen(window, hotkey):
    print(json.dumps(hotkey.getCurrentMap()))
    window.evaluate_js(f"window.updateMapping({json.dumps(hotkey.getCurrentMap())})")
